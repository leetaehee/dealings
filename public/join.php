<?php
	...
	try {
		...
		$title = TITLE_JOIN_MENU.' | '.TITLE_SITE_NAME; // 템플릿에서 <title>에 보여줄 메세지 설정

		$action_url = MEMBER_PROCESS_ACTION.'/member_process.php'; // form 전송시 전달되는 URL.
		$action_mode = 'add'; // 회원가입 모드- 'add', 회원수정- 'modi', 회원탈퇴-'del'
		
		ob_Start();
		include __DIR__.'/../templates/join.html.php'; // 템플릿
		$output = ob_get_clean();
	}catch(Exception $e) {
