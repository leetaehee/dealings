<?php
	...
	try {
		...
		$title = TITLE_EVENT_ENDED_STATUS . ' | ' . TITLE_ADMIN_SITE_NAME;
		...
		if ($eventList === false) {
			throw new Exception('종료된 이벤트 목록을 가져오면서 오류가 발생했습니다.');
		}
		$eventListCount = $eventList->recordCount();

		// 이벤트 결과 저장 보여주는 페이지
		$issueEventResultURL = SITE_ADMIN_DOMAIN . '/event_ended_result.php'; 

		$templateFileName =  $_SERVER['DOCUMENT_ROOT'] . '/../templates/admin/event_ended_list.html.php';
	} catch (Exception $e) {
